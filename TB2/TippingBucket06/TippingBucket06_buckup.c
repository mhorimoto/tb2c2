...
////// Flow Meter setup
#define	r_bucket		0.1826				// Capacity of Right Bucket
//ver1:0.19925 ver2:0.1944
#define	l_bucket		0.1785				// Capacity of Left Bucket
//ver1:0.1906 ver2:0.179
const	float 		bucket_flow =  0.18055;// Average Capacity of Bucket (r_bucket+l_bucket)/2
//ver1:0.194925 ver2: 0.1867
static	long		g_tcnt;				// Timer0 overflow counter
static	int			g_tx_flag;			// Data Request-to-Send Flag
static	long		g_tcnt_ms;			// Time measurement of a RSW pulse interval
static	long		g_tcnt_result;		// The latest RSW pulse cycle
...
void main()
{
	...
	while(1)							
	{
		...
		if(g_cal_flag == 1){
			if(g_tcnt_result < 1000 ){
				bucket_offset =  0.0982;
			...
			else if(g_tcnt_result > 60000){
                //bucket_offset =  0.00388;
				//g_drainage_flow = g_drainage_flow + bucket_flow + bucket_offset;
				...
				bucket_offset = (338.3 * bucket_flow+0.00388*(float)g_tcnt_result) / ((float)g_tcnt_result-338.3);
				g_drainage_flow = g_drainage_flow + bucket_flow + bucket_offset;
				Pulse_flow = g_before_offset + bucket_flow + bucket_offset;
				//printf("1.0sec< <60sec\n"); ////debug
			}
		...
		printf(lcd_data,"Count= %5lu",drainage_cnt);
